...
**もっと簡単に始めたい方は、自動セットアップスクリプトを使えます！**

1. エクスプローラーで `streamlit-llm-app` フォルダを開く
2. `setup.bat`（Windows）または `setup.sh`（Mac/Linux）をダブルクリック
3. 画面の指示に従う

> **注意:** バッチファイルのメッセージは英語で表示されます（文字化け防止のため）。
> 日本語の詳しい説明は「セットアップ手順.txt」をご覧ください。
...
これで STEP 3〜4 をスキップできます。STEP 5（APIキー取得）に進んでください。

---

## 🔧 STEP 3: 仮想環境を作成する（手動セットアップ）

**上記の自動セットアップを使った場合、この手順はスキップしてください。**
...
1. エクスプローラーで `streamlit-llm-app` フォルダを開く
2. `run.bat`（Windows）または `run.sh`（Mac/Linux）をダブルクリック
3. ブラウザが自動的に開きます
...
- `run.bat`（Windows）または `run.sh`（Mac/Linux）をダブルクリックするだけ！
